...
class NeoAttack(pl.LightningModule):
    def __init__(self, hparams): # the hparams here is the parsed config
        super(NeoAttack, self).__init__()
        ...
        # Lightning stores all the args in hparams (also for checkpoint) 
        ...
        # Dataframe that stores loss(perplexity) for individual samples from pile.csv
        self.valid_perplexity_df = None
        self.perp_idx = None
    ...
    # active training loop in torch lightning
    def training_step(self, batch, batch_idx):
        loss, score = self._step(batch)
        self.log('train_loss', loss, on_step=True,
                 on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
        # the train step here tries 
    ...
    def validation_epoch_end(self, output):
        if self.hparams.mode in ['unlearn']:
            ...
            if len(self.hparams.valid_sets) > 1:
                ...
                perp_outputs = self.all_gather(output)[self.perp_idx]
            else:
                outputs = self.all_gather(output)
            keys = outputs[0].keys()  # [doc_id, loss, acc, el]
            full_output = {k: [] for k in keys}

            perp_keys = outputs[0].keys()
            perp_full_output = {pk: [] for pk in perp_keys}
            ...
            for perp_out in perp_outputs:
                for pk in perp_full_output:
                    perp_full_output.append(torch.flatten(perp_out[pk]))
            ...
            ## - perp
            for pk in perp_keys:
                perp_full_output[pk] = torch.cat(perp_full_output[pk])
                perp_full_output[pk] = torch.flatten(perp_full_output[pk]).cpu().numpy()

            if len(perp_full_output['preds'].shape) == 1:
                perp_full_output['preds'] = self.tokenizer.decode(
                    perp_full_output['preds'])
            else:
                perp_full_output['preds'] = self.tokenizer.batch_decode(
                    perp_full_output['preds'])
            ...
            for pk in list(perp_keys):
                perp_full_output[f'{pk}_{log_col_name}'] = perp_full_output.pop(pk)
            perp_full_output['doc_id'] = perp_full_output.pop(f'doc_id_{log_col_name}')
            perp_df = pd.DataFrame(perp_full_output)
            ...
            perp_df['doc_id'] = perp_df['doc_id'].astype(int)
            perp_df = perp_df.drop_duplicates(['doc_id'])
            perp_df = perp_df.set_index('doc_id')
            self.valid_perplexity_df = self.valid_perplexity_df.combine_first(perp_df)
            self.valid_perplexity_df = self.valid_perplexity_df.reindex(self.valid_perplexity_df_index)
    ...
    def on_validation_end(self):
        if self.hparams.mode in [
            ...
            self.init_validation = False
        # we also want some logs for validation sets when init validation is false
        self.valid_perplexity_df.to_csv(f'outputs/validation_log_{self.hparams.wandb_run_name}.csv')
    ...
    # get perplexity (according to the LM) of all entries in a dataset
    ...
    def train_dataloader(self):
        ...
        # only unlearing requires training and requires a length
    ...
    def val_dataloader(self):
        ...
        if self.mode in ['unlearn'] and self.valid_df is None:
            ...
            self.valid_df = self.valid_df.set_index('doc_id') # set the row label to be doc_id
        ...
        # Log dataframe for all validation datasets
        # if self.mode in ['unlearn'] and self.valid_perplexity_df is None:
        #     for i in range(len(self.hparams.valid_sets)):
        #         if i != self.hparams.valid_type_path.index('target'):
        #             self.valid_perplexity_df = datasets[i].dataset
        #             self.valid_perplexity_df = self.valid_perplexity_df.set_index('doc_id') # set the row label to be doc_id
        #             self.valid_perplexity_df_index = self.valid_perplexity_df.index
        #             # The reference prefix for logging Table 3.
        #             self.valid_perplexity_df['prefix'] = self.valid_perplexity_df['text'].apply(
        #                 lambda x: self.tokenizer.decode(self.tokenizer.encode(x)[:100]))
        if self.mode in ['unlearn'] and self.valid_perplexity_df is None:
            self.perp_idx = self.hparams.valid_type_path.index('ppl')
            self.valid_perplexity_df = datasets[self.perp_idx].dataset
            self.valid_perplexity_df = self.valid_perplexity_df.set_index('doc_id') # set the row label to be doc_id
            self.valid_perplexity_df_index = self.valid_perplexity_df.index
            # The reference prefix for logging Table 3.
            self.valid_perplexity_df['prefix'] = self.valid_perplexity_df['text'].apply(
